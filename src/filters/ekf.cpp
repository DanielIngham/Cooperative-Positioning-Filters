...
void EKF::correction(EstimationParameters &ego,
  ...
  ego.innovation_covariance =
      ego_meas_Jacobian * ego.error_covariance * ego_meas_Jacobian.transpose() +
      joint_sensor_noise;
...
void EKF::correction(EstimationParameters &ego_robot,
  ...
  augmentedMeasurementJacobian_t const &measurement_Jacobian{
  ...
  state_estimate += kalman_gain * ego_robot.innovation;
